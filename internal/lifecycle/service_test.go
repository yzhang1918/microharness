...
func TestReopenClearsStaleCIAndSyncSignals(t *testing.T) {
	...
	if _, err := runstate.SaveState(root, "2026-03-18-archive-smoke", &runstate.State{
		...
		Sync:          &runstate.SyncState{Freshness: "stale", Conflicts: true},
		LatestPublish: &runstate.Publish{AttemptID: "publish-1", PRURL: "https://github.com/yzhang1918/microharness/pull/99"},
	}); err != nil {
...
func TestReopenRejectsLandCleanupInProgress(t *testing.T) {
	...
	land := svc.Land("https://github.com/yzhang1918/microharness/pull/99", "abc123")
	if !land.OK {
...
func TestLandUsesLegacyEvidenceCachesWhenPointersAreMissing(t *testing.T) {
	...
	if _, err := runstate.SaveState(root, "2026-03-18-landed-plan", &runstate.State{
		...
		LatestPublish:  &runstate.Publish{AttemptID: "publish-legacy", PRURL: "https://github.com/yzhang1918/microharness/pull/99"},
		Sync:           &runstate.SyncState{Freshness: "fresh", Conflicts: false},
		LatestEvidence: nil,
	}); err != nil {
		t.Fatalf("save legacy state: %v", err)
	...
	}.Land("https://github.com/yzhang1918/microharness/pull/99", "")
	if !result.OK {
...
func TestLandRejectsOverwriteDuringCleanup(t *testing.T) {
	...
	first := svc.Land("https://github.com/yzhang1918/microharness/pull/99", "abc123")
	if !first.OK {
		t.Fatalf("expected initial land success, got %#v", first)
	}

	second := svc.Land("https://github.com/yzhang1918/microharness/pull/100", "def456")
	if second.OK {
	...
	if state == nil || state.Land == nil || state.Land.PRURL != "https://github.com/yzhang1918/microharness/pull/99" || state.Land.Commit != "abc123" {
...
func TestLandAllowsCommitEnrichmentDuringCleanup(t *testing.T) {
	...
	first := svc.Land("https://github.com/yzhang1918/microharness/pull/99", "")
	if !first.OK {
		t.Fatalf("expected initial land success, got %#v", first)
	}

	second := svc.Land("https://github.com/yzhang1918/microharness/pull/99", "abc123")
	if !second.OK {
