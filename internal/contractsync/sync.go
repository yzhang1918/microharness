...
// Sync generates the checked-in contract schemas and reference docs.
...
func expectedFiles(workdir string) (map[string][]byte, error) {
	...
	expected := make(map[string][]byte, len(entries)*2+2)
	docEntries := make([]docIndexEntry, 0, len(entries))
	...
	for _, entry := range entries {
		...
		docPath := docPathForEntry(entry)
		docBytes, err := renderEntryDoc(entry, docPath, schemaBytes)
		if err != nil {
			return nil, fmt.Errorf("render doc %s: %w", entry.Key, err)
		}
		expected[docPath] = docBytes
		...
		docEntries = append(docEntries, docIndexEntry{
			Group:       entry.Group,
			Title:       entry.Title,
			Description: entry.Description,
			SchemaPath:  filepath.ToSlash(entry.Path),
			DocPath:     filepath.ToSlash(docPath),
		})
	}
	...
	sort.Slice(docEntries, func(i, j int) bool {
		if docEntries[i].Group == docEntries[j].Group {
			return docEntries[i].Title < docEntries[j].Title
		}
		return docEntries[i].Group < docEntries[j].Group
	})
	...
	expected["docs/reference/contracts/README.md"] = renderDocIndex(docEntries)
	return expected, nil
}
...
func docPathForEntry(entry contracts.SchemaEntry) string {
	group := strings.ReplaceAll(entry.Group, "_", "-")
	fileName := strings.ReplaceAll(strings.TrimSuffix(filepath.Base(entry.Path), ".schema.json"), ".", "-") + ".md"
	return filepath.ToSlash(filepath.Join("docs", "reference", "contracts", group, fileName))
}

func renderEntryDoc(entry contracts.SchemaEntry, docPath string, schemaBytes []byte) ([]byte, error) {
	var raw map[string]any
	if err := json.Unmarshal(schemaBytes, &raw); err != nil {
		return nil, err
	}

	root := resolveRootSchema(raw)
	defs := mapFromAny(raw["$defs"])

	var b strings.Builder
	fmt.Fprintf(&b, "<!-- Generated by scripts/sync-contract-artifacts. DO NOT EDIT. -->\n\n")
	fmt.Fprintf(&b, "# %s\n\n", entry.Title)
	fmt.Fprintf(&b, "%s\n\n", entry.Description)
	fmt.Fprintf(&b, "- Schema JSON: [%s](%s)\n", filepath.Base(entry.Path), relativeDocLink(docPath, entry.Path))
	fmt.Fprintf(&b, "- Registry entry: [%s](%s)\n", "schema/index.json", relativeDocLink(docPath, "schema/index.json"))
	fmt.Fprintf(&b, "- Go type: `%s.%s`\n\n", entry.Type.PkgPath(), entry.Type.Name())

	renderSchemaSection(&b, "Root Shape", root)
	...
	if len(defNames) > 0 {
		...
		for _, name := range defNames {
			renderSchemaSection(&b, name, mapFromAny(defs[name]))
		}
	}
...
func relativeDocLink(fromDoc, toPath string) string {
	fromDir := filepath.Dir(fromDoc)
...
func renderDocIndex(entries []docIndexEntry) []byte {
	var b strings.Builder
	b.WriteString("<!-- Generated by scripts/sync-contract-artifacts. DO NOT EDIT. -->\n\n")
	b.WriteString("# Contract Reference\n\n")
	b.WriteString("This directory is generated from the Go-owned contract module in `internal/contracts`. ")
	b.WriteString("Use `scripts/sync-contract-artifacts` to refresh these docs and the checked-in JSON Schemas under `schema/`.\n\n")
	b.WriteString("- Schema index: [`schema/index.json`](../../../schema/index.json)\n")
	b.WriteString("- Source of truth: `internal/contracts`\n\n")

	lastGroup := ""
	for _, entry := range entries {
		if entry.Group != lastGroup {
			fmt.Fprintf(&b, "## %s\n\n", prettyGroupName(entry.Group))
			lastGroup = entry.Group
		}
		description := strings.TrimSuffix(entry.Description, ".")
		fmt.Fprintf(&b, "- [%s](%s): %s. Schema: [%s](%s)\n",
			entry.Title,
			relativeDocLink("docs/reference/contracts/README.md", entry.DocPath),
			description,
			filepath.Base(entry.SchemaPath),
			relativeDocLink("docs/reference/contracts/README.md", entry.SchemaPath),
		)
	}
...
func prettyGroupName(group string) string {
	switch group {
