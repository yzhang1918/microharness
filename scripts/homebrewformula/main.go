...
func renderFormula(repo, tag, version string, checksums map[string]string) (string, error) {
	...
	builder.WriteString("  end\n")
	builder.WriteString("\n")
	builder.WriteString("  require \"json\"\n")
	builder.WriteString("\n")
	builder.WriteString("  test do\n")
	builder.WriteString("    output = shell_output(\"#{bin}/harness --version\")\n")
	builder.WriteString("    parsed = JSON.parse(output)\n")
	builder.WriteString("    assert_equal \"v#{version}\", parsed[\"version\"]\n")
	builder.WriteString("    assert_equal \"release\", parsed[\"mode\"]\n")
	builder.WriteString("  end\n")
	builder.WriteString("end\n")
