...
4. Update the tracked plan's durable summaries from those artifacts:
   ...
   - `Archive Summary`
   - `Outcome Summary`
   - for lightweight work, the plan lives under `.local/harness/plans/...`,
     but the same summary refresh rule still applies before archive
...
1. If the profile is `standard`, commit the archive move and summary updates.
2. Push the branch.
